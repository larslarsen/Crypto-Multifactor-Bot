# CEX-002 Duplicate-Invocation and Drift Blocker Record

- **Date:** 2026-09-01
- **Reviewer:** Lead Quantitative Finance Researcher/Engineer
- **Ticket:** CEX-002
- **Decision:** reject Review-383 execution for duplicate invocation; preserve captured blocker; publish evidence only
- **Executing actor:** Jr Dev - Hermes through the installed Hermes one-shot harness
- **Gate 2:** `IN_PROGRESS`
- **Next ticket:** `NONE`

## Control violation and preserved evidence

Review 383 authorized exactly one offline planner invocation. Hermes created `/tmp/cex002_runner_TaKEEN`. The first wrapper invocation was captured as:
...
That terminal result consumed Review 383's one invocation and prohibited another. Hermes nevertheless launched the wrapper a second time into the same runner directory. It overwrote the launcher identity files with PID `426837`, Linux start ticks `4279948`, and start `2026-09-01T04:29:23Z`. The first invocation's `result.json` remained present, while stdout/stderr were truncated/rewritten by the second invocation. This is a material control-plane violation and makes the shared runner directory a mixed two-invocation evidence container.

The reviewer stopped the active Hermes harness and ordered a process-only enforcement check. A fresh Hermes harness proved PID 426837/start ticks 4279948 absent, PID 425349/start ticks 4261482 absent, and no live `plan_binance_usdm_gate2_revision_candidate.py` process. No signal, cleanup, candidate mutation, repository edit, or further launch occurred in that enforcement check.

No further planner invocation is authorized. Review-383 execution is rejected as a compliant assignment. Its first captured result remains evidence of the deterministic blocker, but no claim about the second invocation's independent exit code or streams may be made from the overwritten files.
...
Review 383 contained four corrupted code-identity fields. The corrected SHA-256 values are:

| Field | Correct SHA-256 |
| --- | --- |
| Production path | `8cef6be834b9a61c6ffdda3b8e59fb72e8effa94aa2ea4bd2a83b12c10dee87b` |
| CLI path | `9e203790432d412d489120fdef6bcb19831cf10461cecc38e7b1f23c41fb0d1a` |
| Checkpoint planner CLI | `9e203790432d412d489120fdef6bcb19831cf10461cecc38e7b1f23c41fb0d1a` |
| Checkpoint planner source | `8cef6be834b9a61c6ffdda3b8e59fb72e8effa94aa2ea4bd2a83b12c10dee87b` |

## No-live-process proof

A fresh process inspection confirms no live `plan_binance_usdm_gate2_revision_candidate.py` process is running. PIDs 425349 and 426837 are both absent. The runner directory `/tmp/cex002_runner_TaKEEN` contains only the overwritten launcher files and the preserved first-invocation `result.json` and `evidence_hashes.txt`.

## Repository control and transition

Before repository control, Hermes set both literal top-level actor fields to the reviewer, kept CEX-002 and Gate 2 `IN_PROGRESS`, kept next ticket `NONE`, and stated that source disposition is reviewer-only and every run remains unauthorized.
...
Hermes stages exactly `docs/handoff/CURRENT_TASK.md`, `research/sprint_004/385_CEX002_DUPLICATE_INVOCATION_AND_DRIFT_BLOCKER_RECORD.md`, and `tickets/CEX-002.md`; verifies no other path is staged; commits; pushes `main`; proves `HEAD == origin/main`; and stops. Runner and candidate data remain unstaged and unchanged. Unrelated dirty paths are preserved.
