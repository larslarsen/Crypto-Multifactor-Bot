# CEX-002 Real Typed Source Acceptance and Execution

- **Date:** 2026-08-23
- **Reviewer:** Lead Quantitative Finance Researcher/Engineer
- **Ticket:** CEX-002
- **Decision:** review-251/254 typed-source correction accepted for integration and execution
...
The reviewer inspected Claude Build's final test-only correction once with the accepted
Spark production change at:
...
The complete review-251 typed-source correction is accepted:

- retained book-depth UTC text is normalized through the existing exact integer-calendar
  converter to required non-nullable Arrow `int64` epoch milliseconds;
- exactly the four empirically nullable metrics ratios allow whitespace-empty source
  tokens to become typed nulls, while every nonempty token remains strictly converted;
- required metrics identity and OI level/value fields remain non-nullable and fail closed;
- no source row, object, field, or valid value is removed, inferred, zero-filled,
  forward-filled, rounded, or converted through binary float;
- a valid eight-field four-row metrics fixture proves mixed present/null patterns, exact
  `decimal128(38,18)` values and nullability, all row counts, and required fields;
- a two-row book-depth fixture proves exact epoch values, non-nullable `int64`, and complete
  row retention;
- invalid nonempty ratios, invalid/empty timestamps, missing required metrics, and the
  restored invalid kline decimal all prove strict structural errors and redaction; and
- every review-249 receipt, sizing, idempotence, capacity, lineage, tamper, collision,
  no-follow, race, content-addressed publication, and v1 protection remains intact.

The reviewer read-only cohort audit in review 251 had already checked all 96 retained
physical objects, 56,259 rows, and 491,411 source cells under this exact contract, finding
1,627 declared nullable cells and zero residual conversion failures.
...
hashes and the 144-function count; stop on any mismatch. Stage exactly the two modified
...
Write `research/sprint_004/256_CEX002_V2_SIZING_TYPED_SOURCE_INTEGRATION_AND_EXECUTION.md`
...
2. `research/sprint_004/256_CEX002_V2_SIZING_TYPED_SOURCE_INTEGRATION_AND_EXECUTION.md`;
