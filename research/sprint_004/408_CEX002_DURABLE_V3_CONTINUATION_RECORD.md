# CEX-002 Durable V3 Continuation Record 408

- **Date:** 2026-09-01
- **Evidence actor:** Jr Dev - Hermes through the installed Hermes one-shot harness
- **Reviewer:** Lead Quantitative Finance Researcher/Engineer
- **Ticket:** CEX-002
- **Decision:** preflight exact; planner launched once; natural terminal exit 2 after transient listing failure; v3 tree partial, no locator/publication
- **Gate 2:** `IN_PROGRESS`
- **Next ticket:** `NONE`
...
|------|----------|----------|-------|
| `HEAD` | `2491226e260d711cedf1868f4a2b6ce9da07153e` | `2491226e260d711cedf1868f4a2b6ce9da07153e` | YES |
| `origin/main` | `2491226e260d711cedf1868f4a2b6ce9da07153e` | `2491226e260d711cedf1868f4a2b6ce9da07153e` | YES |
...
All five literal path hashes match exactly. Both v1/v2 checkpoint and private-index hashes match exactly. The v3 root was absent before launch (created during the continuation run).

## Runner identities

- **Runner directory:** `/tmp/runner_407_KyKikU`
- **Runner files:**
  - `run.sh` — SHA-256 `d5c4a6b07531b52a95414b9d2a8178de11135e5f4f0b5b09e538ce20748ee820`, 2922 bytes
  - `metadata/run_identity.json` — SHA-256 `25627c47a2ff018687d9375f7c7e9825c38063b8ae22e7bb985f603e943744a4`, 445 bytes
  - `metadata/terminal_trailer.txt` — SHA-256 `2a45b791326cbdf2fd56058c8c21e1a878ce891efcef4c9fdc949af02b4c4bbb`, 43 bytes
...
- **Shell PID:** 589947
- **Shell start tick:** 6458981 (field 22 of `/proc/589947/stat`)
- **Planner PID:** 589954
- **Planner start tick:** 6458983 (field 22 of `/proc/589954/stat`)
- **Start UTC:** 2026-09-01T10:32:33Z
- **End UTC:** 2026-09-01T10:48:39Z
- **Command:** `PYTHONPATH=src .venv/bin/python scripts/research/plan_binance_usdm_gate2_revision_candidate.py`

## Terminal outcome

The planner was launched exactly once. After 16 minutes 6 seconds of wall time, the planner reached natural terminal with exit status 2 due to a transient listing request failure. The runner captured the exact exit status and wrote an atomic terminal trailer. The runner was NOT killed by `process.kill` or any signal; it observed the planner's natural exit via `wait`.

- **Exit code:** 2
- **Stop reason:** transient listing request failure (`ERROR: listing request failed transiently`)
- **Wall elapsed:** 966 seconds (16 minutes 6 seconds)
- **Wall allowance:** 14400 seconds (4 hours)
- **Terminal trailer:** `END_UTC=2026-09-01T10:48:39Z\nEXIT_STATUS=2\n`
...
- **checkpoint.json:** 3,780,433 bytes, schema `cex002_gate2_revision_candidate_checkpoint_v3`
- **candidate.lock:** 0 bytes (lock file present)
- **pages:** 2,001 page files across 256 prefix subdirectories
- **tmp/listing.sqlite:** 546,447,360 bytes (private index), SHA-256 `3958a3df30b7eb2a71b47d5b1d2bb6d9c162302e636add78acc3bd1a9473cc28`
...
|-------|-------|
| `pass_id` | `pass_1` |
| `published_pages` | `2001` |
| `discovered_prefixes` count | `1308` |
| `completed_prefixes` count | `1257` |
| `listing_complete` | `false` |
| `cursor_state` | `None` |
| `cursor.prefix` | `data/futures/um/daily/metrics/XAGUSDT/` |
| `cursor.continuation_token` | `null` |
...
| `pass_id` | `pass_2` |
| `published_pages` | `0` |
| `discovered_prefixes` count | `2` |
| `completed_prefixes` count | `0` |
| `listing_complete` | `false` |
| `cursor_state` | `None` |
| `cursor.prefix` | `data/futures/um/daily/bookTicker/` |
| `cursor.continuation_token` | `null` |
| `roots` | `["3e148247d863e92ab1b3b06c45db7ada5653130dff091f12c9006825e68b445d", "fe23d9b950114a21e8c72fe19031885c7cc08c83ffd6d986386b9e4231d79bd2"]` |

Pass 1 stopped in the middle of a paginated aggregate prefix at `data/futures/um/daily/metrics/XAGUSDT/`. Pass 2 was initialized in the checkpoint but issued no published page. A null continuation token inside pass 2's initial cursor does not mean the pass completed.
...
The v3 tree is a partial artifact: pass 1 is incomplete (2001 pages, 1257 completed prefixes, listing_complete=false), pass 2 never issued a page. No cross-pass reachability comparison was performed. No semantic identity was computed. No locator, receipt, manifest, or lineage was published.

## Exactly-once proof

- One runner directory `/tmp/runner_407_KyKikU` was created via `mktemp -d`.
- One shell process (PID 589947, start tick 6458981) launched exactly one planner child (PID 589954, start tick 6458983).
...
- The recorded PIDs (589947, 589954) are no longer live in `/proc`, confirming natural process exit.
...
## Untracked root runner copy defect

Repository status shows an untracked root file `run_continuation_runner.sh` (2922 bytes, SHA-256 `d5c4a6b07531b52a95414b9d2a8178de11135e5f4f0b5b09e538ce20748ee820`). This file is byte-identical to `/tmp/runner_407_KyKikU/run.sh` (confirmed via `diff`). It is an unstaged runner-evidence copy created during launch, NOT one of the 13 unrelated untracked paths. The prior handoff claim that a stale intermediate was removed is INCORRECT — this file exists and is identical to the runner script. It is not one of the publication paths and must remain unstaged.

## Stop

CEX-002 and Gate 2 remain `IN_PROGRESS`. Next ticket remains `NONE`. The single authorized durable continuation reached natural terminal exit 2 after a transient listing request failure. No candidate is accepted. No acquisition, transition, later gate, or next-ticket action is authorized. Every unrelated dirty path remains present and unstaged. Harness output is a handoff aid only; this record is the execution evidence and only a later reviewer record may accept or reject the candidate.
