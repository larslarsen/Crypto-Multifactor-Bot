#!/usr/bin/env python3
"""PAPER-007 — Risk-compliant real as-of paper session for tsmom_14_0 on DATA-004.

Runs a single paper-loop session for the frozen tsmom_14_0 config (lookback=14,
skip=0) on the full usable DATA-04 window (2024-04-01 -> 2026-07-23, weekly
decisions). Registers a new research model artifact id and promotes it to
PAPER_APPROVED with effective_time aligned to the first decision so the paper
observation window is valid throughout.

Produces only research/sprint_004/22_TSMOM_14_0_PAPER_SESSION.json.
Does not mutate artifacts 08-21.
"""
...
MODEL_ARTIFACT_ID = "mod_tsmom_14_0_v1"
LOOKBACK_DAYS = 14
SKIP_DAYS = 0
FACTOR_ID = "tsmom_14_0"

# Deterministic experiment fingerprint for tsmom_14_0 on DATA-004.
FINGERPRINT = hashlib.sha256(
    b"PAPER-007:tsmom_14_0:DATA-004:" + MODEL_ARTIFACT_ID.encode()
).hexdigest()

DATASET_ID = "ds_a17651d5c871656f18c29d50fe96d41fa9f08eee8436b276237f96a679764dcd"
EVIDENCE_REFERENCE = "REVIEW-0197"
...
def _ensure_paper_approved(
    ...
    """Register and promote a new tsmom_14_0 model artifact to PAPER_APPROVED."""
    ...
    if current_state is None:
        cand = PromotionIdentityPayload(
            ...
            config_version="cfg_tsmom_14_0_v1",
            feature_version="feat_tsmom_14_0_v1",
        ...
        registry.register_candidate(cand, reason="PAPER-007 candidate")
    ...
    if current_state == PromotionState.RESEARCH_CANDIDATE:
        ...
        registry.transition_state(
            ...
            reason="PAPER-007 research accepted",
        )
    ...
    if current_state == PromotionState.RESEARCH_ACCEPTED:
        ...
        registry.transition_state(
            ...
            reason="PAPER-007 PAPER_APPROVED",
        )


def main() -> int:
    parser = argparse.ArgumentParser(description="PAPER-007 tsmom_14_0 paper session")
    ...
    print(f"PAPER-007: in-memory bar store loaded for {dataset_id}", file=sys.stderr)
    ...
    print(f"PAPER-007: {MODEL_ARTIFACT_ID} promoted to PAPER_APPROVED", file=sys.stderr)
    ...
    # Also derive from period logs for research OOS transparency.
    ...
    artifact: dict[str, Any] = {
        ...
        "protocol_note": (
            "Single risk-compliant real as-of paper session for tsmom_14_0 on the full "
            "DATA-04 usable window (90-day warmup from bar_start, weekly decisions). "
            "A new model artifact id is registered and promoted to PAPER_APPROVED with "
            "effective_time aligned to the first decision. DATA-04 canonical dataset "
            "quality is REJECTED due to BAR-001 native-1d daily resample policy; "
            "intraday 1d bars are used directly. This is promotion-grade evidence, not LIVE."
        ),
        ...
        "live_eligible": False,
        "live_eligible_note": "PAPER-007 is promotion-grade paper evidence; no LIVE authorization.",
    ...
    out_path = output_dir / "22_TSMOM_14_0_PAPER_SESSION.json"
    out_path.write_text(json.dumps(artifact, indent=2), encoding="utf-8")
    print(f"Paper session artifact written to {out_path}", file=sys.stderr)
