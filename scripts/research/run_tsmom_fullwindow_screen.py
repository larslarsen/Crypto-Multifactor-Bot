#!/usr/bin/env python3
"""EXP-007 — Full-window paper screen of remaining multi-fold TSMOM candidates.

Screens the other EXP-006 configs with 2/3 fold gates (`tsmom_14_3`, `tsmom_60_0`)
on the same continuous full window as PAPER-007 (2024-04-01 -> 2026-07-23, weekly
decisions). Each config gets a distinct research model artifact id and is promoted
to PAPER_APPROVED with effective_time aligned to the first decision. Optional
baseline `tsmom_30_7` included for reference.

Produces only research/sprint_004/23_TSMOM_FULLWINDOW_SCREEN.json.
Does not mutate artifacts 08-22.
"""
...
SCREEN_CONFIGS: tuple[_ScreenConfig, ...] = (
    _ScreenConfig(14, 3, "mod_tsmom_14_3_v1", "feat_tsmom_14_3_v1", "cfg_tsmom_14_3_v1"),
    _ScreenConfig(60, 0, "mod_tsmom_60_0_v1", "feat_tsmom_60_0_v1", "cfg_tsmom_60_0_v1"),
    _ScreenConfig(30, 7, "mod_tsmom_30_7_v2", "feat_tsmom_30_7_v2", "cfg_tsmom_30_7_v2"),
)
...
def _fingerprint(model_artifact_id: str) -> str:
    return hashlib.sha256(
        b"EXP-007:" + model_artifact_id.encode() + b":" + DATASET_ID.encode()
    ).hexdigest()
...
def _ensure_paper_approved(
    ...
    """Register and promote a distinct model artifact to PAPER_APPROVED."""
    fingerprint = _fingerprint(cfg.model_artifact_id)
    current_state = registry.get_current_state(cfg.model_artifact_id)
    if current_state is None:
        cand = PromotionIdentityPayload(
            model_artifact_id=cfg.model_artifact_id,
        ...
        registry.register_candidate(cand, reason="EXP-007 candidate")
    ...
    if current_state == PromotionState.RESEARCH_CANDIDATE:
        ...
        registry.transition_state(
            ...
            reason="EXP-007 research accepted",
        )
    ...
    if current_state == PromotionState.RESEARCH_ACCEPTED:
        ...
        registry.transition_state(
            ...
            reason="EXP-007 PAPER_APPROVED",
        )


def _run_screen_config(
    cfg: _ScreenConfig,
    db_path: Path,
    dataset_id: str,
    universe: list[str],
    ...
    factor_id = f"tsmom_{cfg.lookback_days}_{cfg.skip_days}"
    registry = PromotionRegistry(db_path)
    _ensure_paper_approved(registry, cfg, effective_time=decision_times[0])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="EXP-007 full-window TSMOM screen")
    ...
    for cfg in SCREEN_CONFIGS:
        print(f"EXP-007: screening {cfg.model_artifact_id}...", file=sys.stderr)
        cell = _run_screen_config(
            cfg,
            db_path,
            dataset_id,
            universe,
        ...
        results.append(cell)
        print(
            f"  {cell['factor_id']}: return={cell['total_net_return']:.4f} gate={cell['live_gate_satisfied']}",
            file=sys.stderr,
        )
    ...
    any_gate = any(c["live_gate_satisfied"] for c in results)

    artifact: dict[str, Any] = {
        "experiment_id": "EXP-007",
        "data_mode": "real_asof",
        "protocol": "full_window_paper_screen",
        "protocol_note": (
            "Full-window paper screen of the remaining EXP-006 multi-fold candidates "
            "(tsmom_14_3, tsmom_60_0) plus optional baseline tsmom_30_7 on the same continuous "
            "DATA-04 window as PAPER-007 (2024-04-01 -> 2026-07-23, weekly decisions). "
            "Each config uses a distinct research model artifact id and is promoted to "
            "PAPER_APPROVED with effective_time aligned to the first decision. "
            "DATA-04 canonical dataset quality is REJECTED due to BAR-001 native-1d daily "
            "resample policy; intraday 1d bars are used directly. This is promotion-grade "
            "evidence, not LIVE."
        ),
        ...
        "ranking": [
            {
                "rank": i + 1,
                "model_artifact_id": c["model_artifact_id"],
        ...
        "any_fullwindow_gate": any_gate,
        "live_eligible": False,
        "live_eligible_note": "EXP-007 is a full-window paper screen; no LIVE authorization.",
        "cross_references": [
    ...
    out_path = output_dir / "23_TSMOM_FULLWINDOW_SCREEN.json"
    out_path.write_text(json.dumps(artifact, indent=2), encoding="utf-8")
    print(f"Full-window screen artifact written to {out_path}", file=sys.stderr)
