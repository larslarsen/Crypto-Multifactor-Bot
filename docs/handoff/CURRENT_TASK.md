...
Sr's first correction remains rejected before Jr integration. Although targeted ruff is now clean
and the engine performs a durable capability check at its outer public work boundaries, the live
readiness command still performs no workload and `stage-run` still performs no acquisition. The
correction also continues to trust unauthenticated terminals and caller assertions, leaves attempt
attribution disconnected, and lacks the frozen controller/authenticator/migration/test semantics.

Sr's second correction is also rejected before Jr integration. Its synthetic readiness smoke now calls
`process_one`, but the live CLI still substitutes caller pool births and optional bounds for the pinned
production-root sample, cannot execute independent complete rungs, and does not retain/authenticate the
required evidence. The stage CLI constructs an unattached engine that cannot run the existing production
...
Sr's fourth correction is rejected before Jr integration. It fixes exact-file newline hashes, public
readiness initialization, lock ownership transfer, several root/composite checks, and adds partial controller
and migration tests. However, accepted matrix/source authority remains forgeable, readiness replay/byte/
safety authentication is still asserted rather than reconstructed, production attempts and credit are not
transactionally attributable, stage clocks/storage/sealing violate the frozen rules, and tests preserve
placeholder paths.

Sr's fifth correction is rejected before Jr integration. It adds the public matrix authenticator, exact source
paths, stronger isolation, public readiness work, partial raw attribution, a pre-SQLite OS lock, integer clock
arithmetic, non-daemon control, event/checkpoint identity recomputation, immediate-prior migration authority,
and last stage-directory terminal creation. The submission nevertheless cannot prepare production because it
changes the confirmed policy identity after confirmation, still uses PTB/database-allocation byte proxies,
attributes concurrent engine attempts through one shared mutable context, seals before all controller work is
joined, and neither readiness nor stage authentication reconstructs the frozen complete authority.

Sr's sixth correction is rejected before Jr integration. It makes matrix identity fields mandatory, computes the
birth-bound policy before confirmation, adds live readiness stop requests, same-connection credit reauthentication,
immutable-only stage SQLite, and a partial checkpoint rational. Production attempt attribution is nevertheless
nonfunctional because caller-thread-local context is read by the distinct persistence-owner thread. Matrix facts,
...
Sr's eighth correction is rejected before Jr integration. It reads the real matrix fields, puts attempt authority
in crash journals before networking, seeds the readiness rational in the real stage CLI, blocks a bare forged
first-start file, retains ownership for a live control thread, and recursively walks sealed trees. However, its
raw/attempt transaction fails immediately by assigning the read-only SQLite `commit` method; readiness still
allocates database bytes by a raw-byte-share proxy; ARMED and initialized/READY authority remain unbound; RESUME
...
Sr's fourteenth correction is rejected before Jr integration. It now hashes authority-table schema/index/trigger
definitions without substring exclusions and adds real in-transaction failure injection at every grant boundary.
However, authority row values remain entirely omitted and the digest expectation remains stored only in those same
rows. After dropping a hashed immutability trigger, a coordinated attacker can recompute the new digest, rewrite the
excluded authority row plus its files, and pass authentication. The new tests drop a trigger without coordinating
the row/files, or write an intentionally wrong zero digest, so they do not exercise the authorized forgery case.
...
controller source/test drop and its first fourteen corrections are rejected; no part is accepted for Jr
...
`343e93149167109ce195ff298e6a7051de539696`; `HEAD == origin/main`. Sr delivered a second
correction in all seven authorized files. Reviewed SHA-256 values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `00df7d1fb73a24fd11dfd4797bc28669498cd360f6749ee6b04b5c51329473b4`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
  `72f2202ec460d4a20ce1b28ec6e8f95e2b2d1e7d1e48dc4cae2754d3184a0bd5`;
- `scripts/research/run_uniswap_v2_pair_events_v2_production.py`:
  `5921d5dcbc751defb275b46b24e2c8b4a238798a2d4ab306ca3b24fba9d33349`;
- `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `47399301106357eec9f79f43a4286804eaea5425cd931d53617e99f485df91cb`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `1dc1fbb5ff967b80b2907ca823b1f5675fb5d76b35968bdeafe800d91311fb2b`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
  `f529a2705b0cd03b19fa9d26fb17931a3e957ec04ebd1c85fb92b13813472324`; and
- unchanged `tests/acquisition/test_uniswap_v2_pair_events_v2_migration_0021.py`:
  `c5a0d523d559fa81f575b2c597617d6daf69661a982ec7cb539e11ad8df0dc91`.

The correction is rejected for these blocking defects:

1. The live readiness authority remains caller-substitutable. The CLI requires
   `--pool-births-json`, permits caller `--max-rungs`, and makes the production sample pin optional.
   It supplies no accepted registry parquet/manifest, matrix live/replay terminals, or accepted source
   paths to the executor. The executor's optional authority arguments therefore default to absent and
   its “authentication” again compares frozen constants to themselves before creating state.
2. The implemented workload is not the frozen 128-root workload. `run_with_pools` initializes every root
   produced by a default non-production `PlanConfig` from caller pools; it never streams all 1,858,348
   production roots, retains the lowest 128 `RootFilterPlan` geometries, or initializes exactly those
   eight-address birth-clamped roots. Requiring `len(root_domain_ids) == 128` after full plan initialization
   cannot authenticate the real production authority. The default CLI does not even require that check.
3. Rungs are neither complete nor independent. Each rung processes at most `len(pools) * 4` claims rather
   than all 128 roots and adaptive descendants, and all rungs reuse one database/plan whose prior rung has
   already consumed pending work. Later rungs therefore cannot repeat the same workload. The non-production
   plan also does not exercise the production logs-first/global-header-finalization path required by the
   ADR. `attempts_total` counts calls to `process_one`, not retained provider attempts, and `first_ok` means
   only that no Python exception escaped.
4. Capacity selection still omits the frozen evidence and gates. There is no per-attempt/first-attempt
   reconciliation, zero-network raw replay, split/header/PTB conservation, error-domain classification,
   4096-header/response/retained/wall/spool/worker safety accounting, four consecutive 32-root byte groups,
   `bptb_max`, fixed overhead, provisional 2x storage gate, or exact contiguous-rung selection. Eligibility
   is only `processed >= 1` plus reachability of the partial smoke duration.
5. Readiness sealing/authentication is non-authoritative. The manifest inventories only EVIDENCE and REPORT,
   excluding `readiness.db`, raw objects, receipts, spool, migrations/source identities, and their semantic
   relations; EVIDENCE/REPORT writes are not fsynced. Raw and database state remain mutable after TERMINAL.
   `readiness-authenticate` checks only identity and manifest hash and does not recompute the report/evidence,
   inventory, raw pairing/replay, decisions, byte model, safety facts, credentials, extras, or unknown/orphan
   authority.
6. Production preparation still does not prepare production. It checks only terminal/manifest presence and
   lengths, trusts caller source hashes and unverified selected-rung/report/evidence fields, uses total PTB as
   a byte proxy, and inserts only runtime policy. It does not call the readiness semantic authenticator,
   authenticate registry/matrix/source/path authorities, initialize/replay 1,858,348 production roots, seal
   READY, prove empty network/coverage state, measure actual initialized bytes/free space, or authenticate a
   SQLite checkpoint. Its public API still allows `confirm_policy_id=None`, which the senior test treats as
   successful mutation.
7. `stage-run` cannot run the accepted production engine. It opens `ProductionControlStore` before acquiring
   the OS lock, accepts caller-selected rung JSON and step count instead of the stored policy/permit, creates
   an engine but never calls `attach_existing_plan` or otherwise establishes plan/chain phase, then calls
   `run_until_idle`. That fails at the first readiness check, is converted into a sealed FAILED result, and
   the CLI still returns success. Actual filesystem free space is not supplied; the controller defaults to
   `10**18` bytes.
8. The stage controller is not durable over work. `run_until_idle` blocks the controller loop; lease renewal
   and checkpoint occur only after it returns, guarded by the literal `or True`, so no 60-second renewal/
   checkpoint can occur during a long response or run. Permit active/wall bounds, six-hour/24-hour stops,
   first-start authentication, crash/CRASH/RESUME, wall/monotonic consistency, and stage classification are
   absent. Checkpoints still record zero authoritative counts/roots, absolute monotonic process age, and PTB
   as bytes. Drain still calls `run_until_idle(max_steps=0)`, swallows failure, and can assert DRAINED without
   proving started work and persistence drained.
9. Engine authority remains incomplete. No chain/log/header/retry/scanner path calls `record_stage_attempt`.
   Capability is checked at outer public calls, not transactionally at every provider/persistence/finalization
   boundary. Atomic leaf credit still computes width times all addresses/topics rather than pinned-registry
   birth-clamped PTB, checks only immediate parent/child credit, and is inserted without same-transaction
   durable capability authentication. A separate store credit API remains outside the accepted finalizer.
10. Migration/test evidence remains insufficient. Migration 0021 adds a policy/ordinal stage FK and a stage
    update trigger, but leaf credit can still pair one domain with another domain's leaf receipt, terminal can
    reference another same-plan stage's checkpoint, and `prior_terminal_id` deliberately has no FK. The
    migration tests are byte-identical to the rejected correction and still contain only six shallow cases.
    Production tests were reduced to eight: one smoke rung with four pools, no production pin, no independent
    higher rung or failure class; an engine-less seal; an API prepare that explicitly omits confirmation; and
    only offline CLI plan coverage. The new engine test again mutates private `_plan_id`/`_phase` and proves
    no real READY/capability public lifecycle. The required public stage, attribution/credit, controller,
    tamper/replay, generic non-regression, populated migration, rollback, FK mismatch, trigger, and query-plan
    cases remain absent.
...
### Authorized third correction - same seven files only

Jr Dev - Hermes must first commit and push only this review in `docs/handoff/CURRENT_TASK.md` and
`tickets/DEX-003.md`, excluding the uncommitted controller drop, `opencode.json`, and both untracked
research files. After publication, Sr Dev - Grok Build may correct only the same seven authorized files.
Do not add another summary/smoke facade. The next drop must make these exact public paths authoritative:

- `readiness-run` must accept only canonical accepted registry/matrix/source authorities and fixed frozen
  limits; stream the complete production root iterator, retain the exact lowest 128 root objects, and execute
  each rung independently over those exact geometries through the accepted transport/raw/split/header/
  finalization/replay primitives. It must retain and seal every attempt/raw/receipt/decision/byte fact, apply
  every safety/capacity/reachability/storage rule, and expose one zero-network semantic authenticator that
  reconstructs the result from the complete immutable inventory.
- `production-prepare` must first invoke that authenticator, require exact source/policy confirmation before
  mutation, then use existing public foundation APIs to initialize/authenticate all production roots, empty
  state, READY, SQLite checkpoint, actual bytes, and 2x remaining projection without RPC. No caller hashes,
  policy values, paths, sample limits, or PTB assertions may substitute authenticated authorities.
- `stage-run` must acquire the OS lock before opening mutable production state; load the immutable policy and
  exact permit; attach/authenticate the existing production plan; run with automatic concurrent lease renewal,
  60-second checkpoints and deadline/storage stops; atomically attribute every real attempt/raw and recompute
  registry-birth credit in the accepted finalizer; and implement clean drain/close plus crash/resume and exact
  terminal classification. CLI failure must be nonzero.
- The public immutable stage authenticator must recompute all filesystem/DB/raw/engine/controller semantics.
  Migration 0021 must enforce domain/leaf, stage/checkpoint, and prior-terminal composites. Replace the unchanged
  migration tests and smoke production tests with the full decisive public-path matrix already named twice;
  private phase mutation and injected summary assertions are not evidence. Targeted ruff must remain clean.
...
Jr published the third-correction rejection and fourth-correction authorization at pushed commit
`4d9888fb7566bbaa03bfaf3bcd3596415866a924`; `HEAD == origin/main`. Sr delivered a fourth
correction in all seven authorized files. Reviewed SHA-256 values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `b79912d3bd000032c9a3c565f9d94472030fedad3732503ad6d7475e7c95b3a1`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
  `6714c21b5efeca2dd11376f2bec99a48a50002a4c6832cb8b1437df351e5038c`;
- `scripts/research/run_uniswap_v2_pair_events_v2_production.py`:
  `202bbcbb1d2e087a7cce64a08f29fc86385a740de34477133862f73fde00653b`;
- `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `48eea835dab2faa2184f53ef54e3de954969777fca40cb3497b89b5b12b3390f`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `106b241dfc30f1e2a12588d0b4917d077f2872b743495dde0117d4bd633fe349`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
  `51016662d78734509c06bb3637209c309159df41c97150ea190d006b02601eb5`; and
- `tests/acquisition/test_uniswap_v2_pair_events_v2_migration_0021.py`:
  `52772f593da40c99803ac965406dfe2a9490293e81ef54f327fbb1b74afb6254`.

The correction is rejected for these blocking defects:

1. Accepted matrix/source authority is still forgeable. `authenticate_matrix_authorities` accepts standalone
   three-field JSON objects containing the pinned run/evidence/report strings; it never invokes the accepted
   matrix run authenticator, reads the retained manifests/evidence, verifies cohort 8/provider organizations,
   or reconstructs the matrix result. The senior fixtures intentionally fabricate exactly those three-field
   objects. Source files may use any caller-chosen names and paths; no exact authorized file set/path or pinned
   hash set is enforced.
2. Readiness isolation and hard bounds are incomplete. `assert_isolated_root` applies filename-substring
   heuristics and the CLI supplies no explicit production/raw/spool/controller/registry/matrix forbidden roots,
   so related or overlapping paths are not generally rejected. `active_ns` starts before plan/engine setup,
   not immediately before the first chain request. The frozen 7,200-second wall and aggregate 8 GiB retained-
   byte limit are never enforced, and response/resource decisions are not reconstructed per logical attempt.
3. Rung eligibility remains an aggregate assertion, not complete replay authority. It tests table-wide PENDING,
   sums leaf geometries, and reads mutable in-memory metrics, but does not authenticate every provider attempt,
   raw pair, JSON-RPC response, retry, split conservation, shared-header ownership, leaf/dependency relation, or
   zero-network replay. A malformed/terminal lineage can escape the checks if claimed PTB and summary metrics fit.
4. The readiness byte/storage model is explicitly approximate. Raw bytes are assigned to a group only when a
   domain ID happens to appear in the raw pathname; otherwise they become fixed overhead. Database growth is
   divided equally across four groups. Fixed bytes accumulate across attempted rungs, current immutable
   production-tree bytes are absent, and CLI `--free-bytes` can substitute caller authority for actual free
   space. This is not the frozen exact attributed-byte rational.
5. `authenticate_readiness_run_dir` authenticates file hashes but not their semantics. It never opens a rung
   database or replays raw evidence. A self-consistent handcrafted tree with EVIDENCE/REPORT/MANIFEST/TERMINAL
   and asserted eligible `rung_results` can authenticate without any rung directory, attempt, receipt, raw
   object, split, header, or leaf. It also omits hard-link/non-regular/path-escape rules and permits mutable
   post-terminal artifacts.
6. Production prepare still admits substituted and unauthenticated authority. Caller `source_hashes` or arbitrary
   `source_files` may replace the readiness-sealed map without equality checks. The storage gate again treats
   remaining PTB as bytes. Structural empty-state/replay checks omit required headers/dependencies/coverage and
   complete row semantics. `REGISTRY_BIRTHS.json` is written after the immutable runtime policy, its hash is not
   bound into that policy, and `load_sealed_registry_births` verifies neither file hash, policy ID, exact count,
   nor production anchors, so post-prepare birth tampering can control credited PTB.
7. Stage authority/clocks do not implement the permit contract. The CLI opens `ProductionControlStore`—a mutable
   SQLite connection—before the controller acquires the OS lock. Six-hour stage-1 and later 24-hour boundaries
   are not enforced; `stage1_continuation_eligible` is unused. First-start authority is a pre-creatable mutable
   JSON file, event/checkpoint/store times bypass the injected controller clock, and wall arithmetic uses float
   `total_seconds`. Crash handling appends CRASH then pauses without full prior-state authentication or RESUME.
8. Engine attempt attribution is not real attempt authority. Chain/log/header methods append one later synthetic
   summary using the primary role, attempt 1, zero request/response bytes, and no acquisition/raw IDs after an
   entire dual-provider operation. Primary/secondary calls, retries, no-responses, scanner rejection, and retained
   raw pairing are therefore absent, and the attempt insert is a separate transaction. Capability authentication
   also occurs before the coordinator finalization transaction, not inside it; lease expiry/revocation can race
   the birth-credit insert. Registry births bound to the engine are only required to be nonempty.
9. Checkpoint/drain/sealing remains non-authoritative. Every control iteration reuses one startup free-space
   sample, projection still uses remaining PTB as projected bytes, and exact interval byte/PTB maxima and stage
   classifications are absent. The control thread is daemonized and a timed join can leave it live. The stage
   manifest normally inventories no DB/raw artifacts; terminal creation precedes stage-state transition and lease
   release, causing prohibited post-terminal mutations. The authenticator falls back from immutable SQLite to
   mutable `mode=ro`, does not recompute event/checkpoint/terminal identities or most engine/raw/clock/storage/
   credential semantics, and accepts no recursive production inventory.
10. Migration/test evidence remains non-decisive. The prior-terminal trigger proves only same-plan existence,
    not the immediately preceding stage authority. The “forced rollback” test manually rolls back one insert
    after a successful migration; the “exact query plan” test merely asserts query-plan rows are nonempty; the
    named immutability test omits checkpoints/terminals/credits and complete mutation coverage. Production has no
    valid authority/readiness PASS, semantic replay, prepare success, or real successful stage workload. Its stage
    test deliberately lacks READY and accepts a FAILED terminal. The engine test contains
    `assert ... in {"candidate", None} or True`, and the registry test inspects private coordinator state rather
    than proving atomic authenticated credit.

Targeted ruff passes, repository control passes, and `git diff --check` is clean. Sol ran no pytest,
migration, RPC, production-data mutation, or Git action.

### Authorized fifth correction - same seven files only

Jr Dev - Hermes must first commit and push only this review in `docs/handoff/CURRENT_TASK.md` and
`tickets/DEX-003.md`, excluding the uncommitted controller drop, `opencode.json`, and both untracked research
files. After publication, Sr Dev - Grok Build may correct only the same seven authorized files. The correction
must replace assertions/proxies with the actual frozen public authorities:

- Authenticate the complete accepted matrix live/replay directories with their existing public authenticator;
  require the exact accepted source/migration/CLI file set and hashes; enforce explicit nonoverlap against every
  protected root; and execute each rung under the exact wall/response/retained/worker/safety bounds.
- Make readiness eligibility and its immutable public authenticator reconstruct every attempt/raw/receipt,
  replay, split, header, leaf/dependency/PTB, byte group, fixed/current-tree byte, free-space, capacity, and
  selection fact from complete on-disk evidence. No self-asserted summary or caller free-byte value is authority.
- Bind prepare to the identical readiness source/birth/byte authorities, replay all initialized structural state,
  seal an authenticated arming record including registry births and actual bytes/free space, and leave a durable
  non-armed outcome on failure.
- Acquire the OS lock before opening mutable state. Implement the exact six-hour/24-hour/fourteen-day integer
  clock and crash/RESUME rules. Move per-provider/per-retry attempt attribution with exact raw IDs/bytes/outcomes
  into the real persistence transactions and reauthenticate capability inside the atomic finalizer/credit TX.
  Recompute and authenticate the exact full registry birth authority before any credit.
- Replace PTB byte proxies and static free space with exact checkpoint interval accounting and current disk facts;
  join all controller/engine work before sealing; make terminal creation the last write; recursively bind DB/raw/
  controller state; and implement the complete immutable semantic stage authenticator. Finish immediate-prior
  terminal authority and replace every placeholder migration/engine/production test with the full decisive matrix.
  Targeted ruff must remain clean.

Sr does not run tests or migrations, edit other files/records, use RPC/network, touch production or evidence
data, or perform Git actions. Sr stops for a fresh Sol source review with new hashes. Jr integration/testing and
all live readiness, production initialization/RPC/stages, coverage, publication, downstream, PAPER, LIVE, and
...
Jr published the fourth-correction rejection and fifth-correction authorization at pushed commit
`87ea0fddfefc3eb5bc2244003135d9bdc4f49994`; `HEAD == origin/main`. Sr delivered a fifth
correction in all seven authorized files. Reviewed SHA-256 values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `f85f54cf1431350a9b7eee8a88201947a82459e46e27f247293ef973f6ca5699`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
  `924eb09dd5103dae494999069a32052f46d8fdf4c8d5d2a74f53292007a854d0`;
- `scripts/research/run_uniswap_v2_pair_events_v2_production.py`:
  `272336ea41232f148068c59b77618a9011ea2b0cf8155b99f57fe8e69b45c5df`;
- `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `5577ca080172d3ed63329f230a7fc91b917f23a195ab2e32363ae509a25d19ed`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `cc744cf1e9ac3a17e5010902ca4435b766091090501c45457e7a5ae7601391fe`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
  `90a2ccb371ff38832f6f17bb805e8829e40553604b3ad02a78ff2f23b4dba4ba`; and
- `tests/acquisition/test_uniswap_v2_pair_events_v2_migration_0021.py`:
  `5a5ce955a67a052acf6e4816df967f2022836758dc03aeb9afc04849362e29fd`.

The correction is rejected for these blocking defects:

1. Matrix pinning is still fail-open. The accepted public matrix authenticator is now called, but each exact
   live/replay run, evidence, and report pin is checked only when the extracted value is nonempty. A successful
   authenticator result that omits one identity is accepted and replaced in the readiness plan with the frozen
   constant. Cohort/provider validation checks the locally frozen readiness payload, not authenticated matrix facts.
2. Readiness hard bounds are post-hoc observations, not hard controls. Blocking `authenticate_chain` and
   `run_until_idle` complete before the 7,200-second wall and aggregate 8 GiB retained-byte checks run, so an
   over-bound or hung workload is not stopped and drained at the boundary. The public authenticator trusts sealed
   rung, timing, byte, metric, and selection summaries and performs only shallow DB/leaf/raw-link checks; it does
   not reconstruct all attempts, responses, retries, splits, headers, dependencies, PTB, replay, or selection.
3. The frozen exact storage model is not implemented. Readiness assigns SQLite bytes above an arbitrary
   200,000-byte shell to groups in proportion to PTB, accumulates fixed bytes across attempted rungs, and lets
   the CLI leave current production-tree bytes at zero. Production gates still use remaining PTB as bytes.
   Checkpoints omit raw/spool trees, treat the first full tree as interval growth, retain no maximum across
   readiness plus completed intervals, and fall back to remaining PTB when an interval has no credit.
4. Offline prepare is impossible through its own confirmation contract. It first requires
   `confirm_policy_id == preview_policy_id`, then binds the registry-birth core hash into a rebuilt policy and
   requires the same confirmation to equal the necessarily different policy ID. The CLI exposes no prior preview
   of that final identity. Stage-run also loads `REGISTRY_BIRTHS.json` by embedded policy ID only and never
   recomputes the policy-bound core hash, so same-count birth-map tampering is not rejected at the work boundary.
5. Real attempt attribution is unsafe and fail-open. Every concurrent `_dual_fetch` shares one mutable
   coordinator attempt-context dictionary, so overlapping work can acquire another call's stage/domain/attempt
   identity. Missing context/table and insert errors are silently ignored; unknown provider organizations become
   `primary`. The failed/no-descriptor fallback is guarded by `capability is None`, when its recorder cannot write,
   so no-response attempts disappear. Finalizer reauthentication uses a different connection from its transaction.
6. Controller clocks and crash authority remain incomplete. `FIRST_START_WALL.json` is canonical but freely
   pre-creatable with any valid timestamp and is not bound to durable START/policy authority. The control loop
   checks limits only after a real-time 60-second wait, stage-1 continuation is evaluated before the current
   active delta is credited and does not drive required classification, and crash handling appends CRASH then
   pauses without fully authenticating prior state or appending RESUME under a new permit.
7. Drain/sealing can race live work and its database inventory is non-authoritative. If the control thread
   survives the timed join, code records an error but immediately drains/seals while it remains live. Drain uses
   stale session-start free space. The manifest hashes SQLite before terminal-row, stage-state, and lease-release
   mutations, while the authenticator never checks that database metadata, binds no recursive raw/controller
   inventory, and falls back from immutable SQLite to mutable `mode=ro`.
8. Stage semantic authentication remains insufficient. It verifies only that each credit is positive and
   references a leaf; it does not load authenticated births and recompute exact birth-clamped PTB per credit.
   Attempts are reduced to a count rather than reconciled to every call, exact raw pair, byte, outcome, retry,
   and receipt. Engine split/header/dependency/credential and clock/storage classifications remain unauthenticated.
9. Tests do not prove the frozen public paths. There is no successful authenticated prepare or real successful
   stage workload. First-start rejects only malformed JSON, not a canonical forged timestamp. The stage seal test
   passes because database metadata is not authenticated. Migration rollback manually rolls back a normal insert,
   query-plan coverage asserts only nonempty result lists, and immutability omits checkpoint/terminal/credit rows.

Targeted ruff passes, repository control passes, and `git diff --check` is clean. Sol ran no pytest,
migration, RPC, production-data mutation, or Git action.

### Authorized sixth correction - same seven files only

Jr Dev - Hermes must first commit and push only this review in `docs/handoff/CURRENT_TASK.md` and
`tickets/DEX-003.md`, excluding the uncommitted controller drop, `opencode.json`, and both untracked research
files. After publication, Sr Dev - Grok Build may correct only the same seven authorized files:

- Make every matrix identity mandatory and derive cohort/provider facts from authenticated evidence. Enforce
  readiness wall/retained limits while work is live and join/drain it. Make zero-network readiness authentication
  reconstruct complete attempt/raw/receipt/replay/split/header/dependency/PTB/byte/capacity/selection authority.
- Use exact attributed raw, receipt, database, fixed, and existing-tree bytes per readiness group and carry the
  maximum rational through every production checkpoint. Measure all production trees/current free space and
  remove every PTB-as-byte and proportional/equal-share proxy.
- Compute the final birth-bound policy before confirmation and mutation. Recompute the complete sealed birth
  authority against policy and production anchors at every prepare/stage/credit/auth boundary, and persist a
  durable non-armed failure outcome when preparation cannot arm safely.
- Replace the shared attempt context with concurrency-safe per-operation authority; record every provider/retry/
  no-response attempt with exact raw/byte/outcome identity in the actual persistence transaction and fail closed
  on attribution failure. Reauthenticate capability on the same finalizer connection used for atomic credit.
- Bind first START to durable authority; implement authenticated CRASH plus new-permit RESUME and exact six-hour/
  24-hour/seven-day/fourteen-day classifications. Join all work before seal, make terminal the last write across
  all sealed authority, recursively bind DB/raw/controller state, require immutable SQLite, and fully reconstruct
  attempts, credits, clocks, storage, and engine semantics.
- Replace placeholders with successful public prepare/readiness/stage/auth paths, adversarial concurrency/no-
  response/birth/clock/seal cases, and real populated migration rollback/exact-plan/full immutability coverage.
  Targeted ruff must remain clean.
...
Jr published the fifth-correction rejection and sixth-correction authorization at pushed commit
`4161ce6a83e8f8ca824a27bbdfd5e52869d22396`; `HEAD == origin/main`. Sr delivered a sixth
correction in four of the seven authorized files; migration 0021 and the engine/migration test files are
unchanged. Reviewed SHA-256 values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `a8a00fa43b1e5818123b329365792999589c8252642d5454ca1cb14dba098cd5`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
  `98ae14f2492480c2d38c4a696c7dc919e3d0a5239905bf67179f8781fadae565`;
- `scripts/research/run_uniswap_v2_pair_events_v2_production.py`:
  `a8da59915e8fcf1d5717b44cfdb7d3997b5b7afb3e257a864eb4666cb7337fe4`;
- unchanged `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `5577ca080172d3ed63329f230a7fc91b917f23a195ab2e32363ae509a25d19ed`;
- unchanged `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `cc744cf1e9ac3a17e5010902ca4435b766091090501c45457e7a5ae7601391fe`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
  `2515a015f4dab1486346fc4d90fb19954237ff1ff51b94ffb600560185613c0b`; and
- unchanged `tests/acquisition/test_uniswap_v2_pair_events_v2_migration_0021.py`:
  `5a5ce955a67a052acf6e4816df967f2022836758dc03aeb9afc04849362e29fd`.

The correction is rejected for these blocking defects:

1. Production attempt attribution is now nonfunctional. `_dual_fetch` writes its attempt context to
   `threading.local()` on the caller/node thread, while `_maybe_insert_stage_attempt` reads that thread-local on
   the dedicated persistence-owner thread. The persistence thread therefore sees `ctx is None` and silently
   returns without inserting. The later fallback remains guarded by `capability is None`, exactly when its store
   recorder cannot write, so production provider calls, failures, and no-responses can all leave zero attempts.
   No engine test was added for this cross-thread production path.
2. Matrix cohort/provider authority is still optional. Exact run/evidence/report identities are now mandatory,
   but cohort and provider values are checked only if an optional `live_auth["plan"]` and optional fields happen
   to exist; missing facts pass. Replay cohort/provider facts are not checked at all. The frozen local readiness
   payload remains the fallback assertion rather than authenticated matrix evidence.
3. Readiness stopping and semantic authentication remain incomplete. The watchdog is daemonized, receives only
   a five-second timed join, and merely requests engine stop; code proceeds if it remains live. It does not retain
   an authoritative limit-trigger event. The zero-network authenticator is materially unchanged: it uses mutable
   `mode=ro`, derives selection from sealed `eligible` booleans, and does not reconstruct every provider attempt,
   raw response, retry, split, header/dependency, first-attempt metric, replay, PTB, timing, byte, or selection fact.
4. The exact byte/storage model remains absent. Readiness still declares 200,000 database bytes fixed and divides
   the remainder among groups in proportion to PTB; fixed bytes still accumulate across rungs and CLI readiness
   still supplies zero current production-tree bytes. Production prepare checks only positive free space before
   initialization and twice the initialized tree afterward, with no readiness maximum for future production.
   Checkpoints still omit raw/spool trees, do not seed the readiness rational, and project zero future bytes until
   a positive credited interval happens to establish a local ratio—a fail-open storage gate.
5. Prepare/sealed-birth authority is still operationally incomplete. The final birth-bound policy is correctly
   computed before comparison, but no public preview emits that final ID for confirmation. Mutations begin by
   exclusively writing births, then policy and READY roots; any later initialization/storage failure leaves no
   required durable non-armed outcome and can leave partial authority. The stage authenticator catches every birth
   load/hash failure, sets `births_map = None`, and then skips PTB recomputation, making birth authentication
   explicitly fail-open.
6. Clock/crash/stage classification rules are substantially unchanged. A canonical but forged
   `FIRST_START_WALL.json` remains accepted without binding to a durable START/policy identity. Crash handling
   appends CRASH and pauses without full prior-state authentication or new-permit RESUME. Limits are first checked
   after a real 60-second control wait; stage-1 eligibility is evaluated before the current active delta is added
   and still does not produce the frozen seven-day/fourteen-day continuation classification.
7. The new live-thread comment does not match control flow. After two timed joins, a still-live control thread
   sets `work_exc = ... "refuse seal"`, but `run_stage_session` immediately calls `drain_and_close` and seals a
   FAILED stage anyway. The thread can therefore race engine close, checkpoint, DB mutation, and terminal sealing.
8. Stage sealing/authentication is still not self-consistent or complete. The manifest captures the DB before
   inserting its terminal row, changing stage state, and deleting the lease. Authentication checks only database
   path and byte length—not the recorded SHA-256—so same-length mutations/tampering pass and the stored hash is
   unused. Raw/spool/controller trees are not recursively bound. Attempts receive only row-shape checks, credits
   can skip births as above, and complete engine receipt/replay/credential/clock/storage semantics are not rebuilt.
9. The explicitly required decisive test work was not delivered. Migration 0021 and its tests are byte-for-byte
   unchanged: rollback still manually rolls back a normal insert, query-plan coverage still asserts only
   `all(plans)`, and immutability remains incomplete. Engine tests are unchanged despite the new cross-thread
   attribution and same-connection credit code. Production tests still have no successful final-policy preview/
   prepare, real successful production stage, canonical forged first-start, live-thread refusal, recursive seal,
   complete semantic authenticator, or exact byte-model test.

Targeted ruff passes, repository control passes, and `git diff --check` is clean. Sol ran no pytest,
migration, RPC, production-data mutation, or Git action.

### Authorized seventh correction - same seven files only

Jr Dev - Hermes must first commit and push only this review in `docs/handoff/CURRENT_TASK.md` and
`tickets/DEX-003.md`, excluding the uncommitted controller drop, `opencode.json`, and both untracked research
files. After publication, Sr Dev - Grok Build may correct only the same seven authorized files:

- Carry immutable per-operation attempt authority in the persistence command/descriptor itself, not thread-local
  or shared mutable state. In the persistence-owner transaction, fail closed and record every provider/retry/no-
  response attempt with exact identities/raw/bytes/outcome. Add a real capability-bound cross-thread test that
  proves two concurrent calls cannot swap or lose attribution.
- Require cohort 8 and both provider organizations from complete authenticated live and replay matrix evidence.
  Make the readiness watchdog owned/joined and durably classify the exact trigger. Make immutable zero-network
  readiness authentication reconstruct all attempt/raw/replay/split/header/dependency/PTB/timing/byte/selection
  semantics without trusting summary booleans.
- Replace every byte proxy with exact group raw/receipt/database/fixed/existing-tree attribution. Seal and carry
  the readiness maximum rational into production, measure DB/raw/spool/controller plus current free space at every
  checkpoint, and never project zero when remaining production authority is nonzero.
- Add a public final birth-bound policy preview before mutation. Make prepare failure durably non-armed and fully
  recoverable/atomic. Birth loading and per-credit recomputation must be mandatory and fail closed in stage work
  and authentication, using the durable policy ID plus core hash and exact production anchors.
- Bind first START to durable authenticated control state; implement full prior-state CRASH authentication and
  new-permit RESUME; implement exact active/wall/seven-day/fourteen-day stop classifications. If any work thread
  is live, return/refuse seal without calling drain/seal. Reorder the global seal so its recursively inventoried
  DB/raw/controller identity is captured after every mutable DB action and verified by exact hashes immutable-only.
...
Jr published the seventh-correction rejection and eighth-correction authorization at pushed commit
`11246229eaf2266050c3b4bc4c227105f1e6186e`; `HEAD == origin/main`. Sr delivered an eighth
correction in six of the seven authorized files; migration 0021 source remains unchanged. Reviewed SHA-256
values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `703dcb4a8630917192af9deb2e8b7c286e6cf163010d41a35431024745c833ed`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
...
- unchanged `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `5577ca080172d3ed63329f230a7fc91b917f23a195ab2e32363ae509a25d19ed`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `2f46b65c930b81f6c888ae6a1d35a7c0c94dd0f7e45ff83d3ca0580709b78f1a`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
...
1. Production-bound persistence is nonfunctional before its intended transaction starts. `_op_persist_envelope`
   assigns `conn.commit = _deferred_commit` and later restores it. `sqlite3.Connection.commit` is a read-only
   built-in method on the real connection used by the persistence owner, so this assignment raises
   `AttributeError` for every production descriptor. The new rollback test expects `PairEventV2Error` and therefore
   does not cover the real failure. Even an assignable fake would not establish a supported SQLite transaction
   boundary around catalog methods that explicitly commit.
2. Raw/attempt reconciliation is neither exact nor production-scoped. It runs only when at least one attempt exists,
   ignores FAILED/no-response acquisitions, and deliberately permits failed acquisitions with no attempt link.
   Conversely, once one attempt exists it scans every `raw_acquisition` in the database and requires every
   SUCCEEDED historical acquisition to belong to this stage. The real database already contains substantial
   pre-controller raw history, so a genuine stage seal would reject unrelated evidence. There is still no proof
   that every production acquisition, including crash recovery and failure, maps to exactly one correct attempt.
3. Readiness database/receipt bytes remain a proxy. All four groups execute together in one rung database; afterward
   the code multiplies the entire SQLite file size by each group's raw-byte fraction. That proportional share is not
   measured database row/index growth and cannot be authenticated as exact per-group authority. Existing production
   tree measurement also silently skips stat failures and does not reject overlapping production roots. The positive
   rational and CLI seed are progress, but they are derived from this non-exact model.
4. ARMED/NON_ARMED still is not immutable, transactional network authority. The immutable runtime-policy identity is
   stored before initialized byte counts are known and does not bind `initialized_bytes` or `database_bytes`; those
   fields exist only in mutable `PREPARE_OUTCOME.json`, and `authenticate_armed_prepare` does not require or verify
   them or recompute READY anchors. A failure after READY/policy mutation cannot roll those mutations back, while
   the resulting NON_ARMED file is neither schema-bound nor a recoverable fresh-prepare transaction.
5. First-start and resume semantics remain incomplete. A canonical file is rejected when no START exists, but when
   any same-plan START exists the code checks only plan and wall; stage, policy, permit, and controller bindings are
   not reconciled (the policy mismatch branch explicitly passes). A crash after the exclusive file write but before
   START persistence permanently bricks recovery. No code emits a new-stage `RESUME`, authenticates
   `prior_terminal_id`, or binds the new permit/stage to the crashed terminal; it only records CRASH and asks the
   caller to create another stage.
6. The stage seal is not one durable self-authenticating identity. The durable `core_hash` excludes the database
   metadata, while `full_manifest_hash` is stored only in editable `TERMINAL.json`. Database bytes can therefore be
   changed and both JSON files regenerated without changing the durable terminal core. The recursive inventory also
   skips symlinks during sealing and orphan detection, and resolves a listed path before testing `is_symlink`, so
   symlink substitutions/extras are not rejected. Tree roles/roots are not all re-bound to the runtime policy.
7. Complete semantic authentication remains absent. Attempt checks do not authenticate request/outcome/provider/raw
   metadata or retry identity; readiness authentication still relies materially on sealed summaries instead of
   replaying the retained provider/raw/split/header/leaf relations. The stage-wide raw scan described above is not a
   substitute for stage-scoped production lineage.
8. The decisive tests remain insufficient or misleading. The matrix-shape test only asserts a hand-written dict and
   never calls the public authenticator. The atomic rollback test cannot pass on a real SQLite connection as written.
   NON_ARMED coverage supplies a hand-written file, first-start covers only the no-START case, and the query-plan test
   still accepts any index/table-name fallback for leaf credit rather than the exact required index. There is still
   no successful public final preview/prepare/live-stage chain proving the frozen authorities end to end.

Targeted ruff passes, repository control passes, and `git diff --check` is clean. Sol ran no pytest,
migration, RPC, production-data mutation, or Git action.

### Authorized ninth correction - same seven files only

Jr Dev - Hermes must first commit and push only this review in `docs/handoff/CURRENT_TASK.md` and
`tickets/DEX-003.md`, excluding the uncommitted controller drop, `opencode.json`, and both untracked research
files. After publication, Sr Dev - Grok Build may correct only the same seven authorized files:

- Replace method monkeypatching with a real supported single-connection transaction design. Raw registration,
  failed registration, and stage-attempt insertion must commit or roll back together on ordinary and recovery
  paths. Carry a durable stage-scoped production lineage key that reconciles every production raw acquisition,
  including FAILED/no-response, exactly once without claiming unrelated historical raw evidence.
- Execute/measure readiness groups so each group's raw, receipt, SQLite row/index delta, and fixed overhead are exact
  observed facts rather than proportional allocation. Reject stat/path overlap/errors, authenticate the positive
  maximum rational, and keep the real CLI seed.
- Make PREPARE outcome a durable identity whose ARMED state binds the final policy, births, readiness rational,
  exact initialized DB/tree bytes, READY anchors, and absence of network/coverage state. Preparation failure must
  leave schema-enforced NON_ARMED authority and require a clean new transaction/identity before any later arming.
- Derive first-start from durable START atomically or provide recoverable reconciliation of the exact stage/policy/
  permit/controller identity. Require prior terminal plus a newly authenticated permit/stage and emit/authenticate
  RESUME only there. Preserve the real live-thread ownership correction.
- Use one non-forgeable seal chain that durably binds the post-mutation database SHA and full manifest while avoiding
  self-reference. Bind each recursive root to policy, reject symlinks/hard links/extras before and during auth, and
  reconstruct exact attempt/raw/receipt/credit/clock/storage semantics.
...
Jr published the thirteenth-correction rejection and fourteenth-correction authorization at pushed commit
`949d9fd`; `HEAD == origin/main`. Sr delivered a fourteenth correction in all seven authorized files.
Reviewed SHA-256 values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `0b9c69868d3839f9df857de2da04379f7191a47020d7bd6dfb3a62e0e7b00a49`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
  `07d5d05d3318bffd2760b1e47805ecfc0e16a888b30b092f029fc5227c04da75`;
- `scripts/research/run_uniswap_v2_pair_events_v2_production.py`:
  `d2d838048c3daba57c1fa3be63e195047c52abcb74b269a8a1b248ff7a4a08e6`;
- `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `38a613fd7764aba3b1fa4eac1077f9dc397a5da3f4b9de80f3dfa82cd0e55cf9`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `732611a0f575f259b3ab1464c7fb20560e7c9c5255fa16bfe0ca34c2bc7b615c`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
  `7d7491f58c6d336764373b83d409e5db2b9e0fb83f7ee03c632ab79ace8e4aff`; and
- `tests/acquisition/test_uniswap_v2_pair_events_v2_migration_0021.py`:
  `13cc6213fbee02ae663415c71040f5db0e19f4cd945235030fb11ad43ae506b0`.

The correction is rejected. It correctly removes SQL-substring exclusions, hashes every authority-table schema,
index, and trigger definition, retains deterministic keyed ordinary rows, and adds controlled rollback injection
after stage update, lease write, and RESUME insertion. Those transactional tests prove PREPARED/no-created-lease/
no-RESUME rollback and byte-identical preservation of a pre-existing foreign lease. One architectural circularity
remains:

1. Both authority tables' entire row content is still represented only by the constant marker
   `ROWS_OMITTED_SELF_SEAL`. The expected live digest is stored in those omitted rows and has no independent pinned
   schema authority. Dropping an immutability trigger changes the digest, but after that drop the authority row is
   writable: recompute the new digest, update the omitted row's digest/bytes/full-manifest fields, and rewrite the
   corresponding MANIFEST/TERMINAL or PREPARE JSON. Authentication then compares the forged artifacts to one another
   and accepts. Hashing the trigger is not sufficient when the mutable expected value can be updated to bless its
   absence.
2. The new stage test drops the terminal-seal no-update trigger but leaves the old seal row/files, so a digest
   mismatch is inevitable. The ARMED test drops the baseline trigger but writes `"0" * 64` instead of the actual
   recomputed digest and does not coordinate the ARMED file/tree. Neither test performs the exact trigger-drop plus
   correctly recomputed authority-row/file rewrite required by the fourteenth authorization.

Targeted ruff passes, repository control passes, and `git diff --check` is clean. Sol ran no pytest,
migration, RPC, production-data mutation, or Git action.

### Authorized fifteenth correction - same seven files only

Jr Dev - Hermes must first commit and push only this review in `docs/handoff/CURRENT_TASK.md` and
`tickets/DEX-003.md`, excluding the uncommitted controller drop, `opencode.json`, and both untracked research
files. After publication, Sr Dev - Grok Build may correct only the same seven authorized files:

- Establish a source-pinned or otherwise independent expected authority-schema identity that authentication checks
  before trusting terminal-seal or prepare-baseline rows. The exact required authority table/index/trigger DDL may be
  pinned in production source or derived from a separately immutable migration identity; a rewritten authority row
  must never be able to bless removal or alteration of its own protection.
- Add the full coordinated adversarial tests: drop the terminal-seal trigger, compute the new live digest, rewrite
  every omitted seal/file field consistently, and require stage auth failure; repeat for prepare-baseline plus ARMED
  JSON/tree. Include authority-row deletion/reinsertion where constraints allow. Do not use deliberately incorrect
  placeholder digests as the failure mechanism.
- Preserve the corrected exact schema/key digest, substring handling, atomic grant and injected rollback coverage,
  foreign-lease preservation, START authentication, ARMED baseline, heartbeat, persistence, crash terminal,
  prior-seal authentication, NON_ARMED, readiness, lineage, and migration/query-plan work. Targeted ruff must remain
  clean.
...
Jr published the prior rejection/correction authorization at pushed commit
`7450afc4bdb6a293e6aead9f3f49e7667c2955c6`; `HEAD == origin/main`. Sr delivered a first
correction confined to six of the seven authorized files; the accepted engine-test file remains
unchanged. Reviewed SHA-256 values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `9dceb2109b8f4187522c00c14fa5aa3641086e20b8550611e8631f8922ff9aed`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
  `091ed7a53899d6bab6655a980281e62971b4412bd555b51263895dd81c0c85c4`;
- `scripts/research/run_uniswap_v2_pair_events_v2_production.py`:
  `8dbb9babc1a6149a3d1aefd29d010b68b1c17122b5b0a40472d04759630c1d9f`;
- `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `90a845b598b2bcae20b149e2ed96eed7c76fa98672bdc7e4a48e36f4a141bea4`;
- unchanged `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `a27a80809e953d096489f3adf48c5d203a1a45bc859a51f4c21f6d3523c12ddd`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
  `c5239eacfa9537855b16df43a64cd14d7f17898c3f78581e7adb3dc7c1379d27`; and
- `tests/acquisition/test_uniswap_v2_pair_events_v2_migration_0021.py`:
  `c5a0d523d559fa81f575b2c597617d6daf69661a982ec7cb539e11ad8df0dc91`.

The correction is rejected for these blocking defects:

1. Readiness still does not execute readiness. `run_rungs_with_engine` accepts any caller-provided
   128 strings, does not enforce the pinned sample digest/PTB or reconstruct their geometries, and for
   each rung merely constructs and closes an engine. It performs no initialize, chain call, root/log
   call, split, raw replay, header batch, finalization, or first-attempt reconciliation. A fixed clock
   therefore gives `active_ns=0`, the code substitutes one nanosecond into reachability, and every
   constructed rung can become eligible without one provider response. The CLI requires the forbidden
   caller-supplied `--domain-ids-json` instead of streaming/authenticating the complete registry-root
   authority.
2. Readiness authorities and evidence are not authenticated or sealed. `authenticate_matrix_authorities`
   compares constants to themselves and reads no accepted registry, matrix terminal, evidence tree, or
   source file. The run writes only a mutable `TERMINAL.json` with no manifest, receipt/raw inventory,
   evidence/report identity, four-group byte model, resource/safety metrics, fsync/clean-close proof, or
   credential scan. `readiness-authenticate` checks only that this JSON repeats `READINESS_IDENTITY`.
3. `stage-run` is not an acquisition controller. It opens mutable production SQLite before taking the OS
   lock, moves the stage to RUNNING, creates a lease/START event, prints a note, closes the store, and
   exits. It never constructs/configures/binds the engine, authenticates chain state, runs work, renews
   capability, checkpoints, stops, drains, closes, or seals; it leaves a RUNNING stage and unexpired
   durable lease after the process releases its OS lock.
4. Stage-attempt authority remains disconnected. No engine path calls `record_stage_attempt`, so chain,
   log, header, no-response, retry, and scanner-rejected attempts are unattributed. Capability is checked
   only at outer method entry, not again at each network/persistence/finalization boundary, and credit is
   inserted later in the finalization transaction without durable capability reauthentication there.
   Leaf PTB is simple domain width times address/topic count rather than recomputed against pinned registry
   births; only immediate parent/child rows are checked, and the separate caller-PTB `credit_leaf` API
   remains available.
5. Production preparation trusts a forgeable JSON file. It accepts any `COMPLETE_PASS` terminal carrying
   the pinned identity, synthesizes missing run/evidence/report hashes, accepts arbitrary caller source
   hashes, and writes the runtime policy before the CLI checks an optional policy confirmation. It does
   not run the public readiness authenticator, authenticate matrix/registry/source/path authorities,
   stream/init/authenticate 1,858,348 roots, seal READY, prove empty network/coverage state, reconcile
   actual initialized bytes, or checkpoint/authenticate the database and exact 2x byte projection.
6. Controller clocks/checkpoints do not implement the contract. First-start authority is a mutable loose
   JSON file rather than authenticated durable state; event/store wall time bypasses the injected clock;
   wall arithmetic uses float conversion; no 60-second checkpoint/renewal loop, missed-deadline handling,
   crash/RESUME path, permit active/wall boundary, six-hour stop, or later 24-hour boundary exists.
   Checkpoints still assert zero leaf/raw/node/terminal/backlog counts, empty evidence roots, absolute
   process monotonic time as credited active time, and remaining PTB as a byte proxy.
7. Lease/drain/seal remains unsafe. The public lease API still exposes `steal=True`; lease loss does not
   trigger the controller stop sequence. Drain calls `run_until_idle(max_steps=0)`, swallows any failure,
   and can record DRAINED without proving every started operation/persistence item closed. It computes the
   terminal event root before END, inserts the terminal before END/state transition/lease release, creates
   TERMINAL before those durable mutations, omits directory fsync and logical DB/raw inventory, and has no
   enforcement against post-terminal writes.
8. The stage authenticator is not semantic or immutable. A wrong nonempty `report_hash` is explicitly
   accepted; event/payload/checkpoint/terminal identities are not recomputed; the terminal root is not
   reconciled to END; permits, policies, clocks, projections, credits, registry births, raw pairs/bytes,
   engine candidates/headers/leaves/dependencies, credentials, hard links, recursive inventory, and
   unknown/orphan authority are not authenticated. SQLite opens with `mode=ro`, not immutable read-only.
9. Migration 0021 still permits authority mismatches. A stage can combine one same-plan policy with a
   permit for another policy/ordinal; leaf credit can combine a domain with another domain's leaf receipt;
   a terminal can reference another stage's checkpoint within the same plan; and `prior_terminal_id` has
   no FK. The mutable stage table has no schema trigger protecting immutable identity/baseline fields or
   the frozen transition graph. The public `steal` mutation is also not a permitted coordinator rule.
10. Tests remain non-decisive: only twelve production tests and six migration tests were added, while the
    authorized engine-test file is unchanged. The readiness test asserts PASS after engine construction
    without transport calls; prepare consumes that synthetic terminal; the CLI test checks only the absent
    live flag; the attempt test calls the detached store API; and the migration “all control tables”
    immutability loop tests only runtime policy. No required public workload, stage lifecycle, attribution/
    credit, crash/clock/storage/drain, semantic tamper/replay, generic non-regression, populated authority,
    rollback, composite mismatch, or exact query-plan case exists.

Targeted ruff passes, repository control passes, and `git diff --check` is clean. Sol ran no pytest,
migration, RPC, production-data mutation, or Git action.

### Authorized second correction - same seven files only

Jr Dev - Hermes must first commit and push only this review in `docs/handoff/CURRENT_TASK.md` and
`tickets/DEX-003.md`, excluding the uncommitted controller drop, `opencode.json`, and both untracked
research files. After publication, Sr Dev - Grok Build may correct only the same seven authorized files.
This is a functional public-path correction, not permission to weaken ADR-0015 or add parallel private
semantics:

- Make the exact CLI readiness path derive the pinned sample from authenticated registry/root authority
  and actually execute/replay all 128 geometries through the accepted public engine primitives on every
  attempted rung. Remove caller domain substitution. Authenticate retained matrix/source authorities,
  classify every frozen outcome/gate, compute exact group bytes/selection, cleanly drain, and seal a
  zero-network-reproducible immutable terminal. Synthetic tests must exercise calls, attempts, splits,
  headers, raw replay, selection classes, and safety failures rather than engine construction.
- Make offline prepare consume only that full public authenticator's COMPLETE/PASS result and exact sealed
  hashes; initialize/authenticate the full production foundation and READY manifest with no RPC; prove
  empty pre-network state; bind exact policy/source/path identities; checkpoint SQLite; and apply the
  authenticated byte model/storage gate before atomic arming. Confirmation must precede mutation.
- Make `stage-run` own the complete gated controller lifecycle: OS lock before mutable state, exact permit/
  policy/clock/deadline authentication, engine construction/binding, automatic lease renewal and 60-second
  durable checkpoints, per-boundary capability/attempt attribution, authoritative atomic birth-clamped
  credit, stop/drain/close/crash/resume, and terminal sealing. Remove lease theft and caller PTB authority.
- Make sealing and the public immutable authenticator recompute the complete filesystem/DB/raw semantics
  and every hash, identity, clock, counter, projection, credential scan, relation, and unknown/orphan rule
  frozen in section 9.11. No mismatch may be accepted merely because a field is nonempty.
- Fix migration 0021 with composite policy/permit/ordinal, domain/leaf/stage/checkpoint/prior-terminal
  authority and schema-enforced identity/transition rules. Add the full decisive engine, production/CLI,
  and populated migration/rollback/index test matrix named in the original authorization. Targeted ruff
  must remain clean.
...
The correction closes important integration failures. A stable acquisition ID now reaches physical
publication and deferred catalog registration; raw catalog, attempt, and lineage writes share the
short owner transaction. The public controller binds capability, creates START, attaches READY,
authenticates chain state, runs work, and drains only tracked started work. Readiness JSON errors are
normalized, the CLI rejects root symlinks before resolution, the migration uses nonredundant named
UNIQUE indexes selected by natural production query shapes, and several invalid fixtures now build
real START/crash/terminal/RESUME authority. Those changes are retained. Acceptance is nevertheless
blocked by the following explicit sixteenth-scope failures:

1. The claimed commit-before-cleanup startup proof does not exercise that crash window.
   `test_production_journal_recovery_carries_authority_and_is_idempotent` lets the first engine clean
   both spool and journal, then starts a second engine with nothing to replay. The only failed replay
   test calls direct persistence twice rather than recovering through startup. Source cleanup also
   unlinks the spool before the journal; a crash between those unlinks makes startup reinterpret a
   previously committed success as a missing-spool failure. Malformed production-authority field
   conversion can still fall through the generic per-journal `continue`, allowing production startup
   with unresolved evidence. Exact success and failure recovery is therefore neither proven nor
   fail-closed at every cleanup boundary.
2. Existing attempt rows are not authenticated byte-for-byte. `_maybe_insert_stage_attempt` selects
   status/acquisition/raw/outcome but compares only plan, stage, provider organization, logical call,
   and attempt number before returning the existing row. Provider role, domain, block, start/end time,
   status, request/response bytes, outcome JSON, acquisition/raw IDs, and created time may diverge and
   be silently accepted. Existing lineage similarly is not checked against its recomputed deterministic
   lineage ID. No adversarial startup test mutates each immutable acquisition/attempt/lineage field and
   proves refusal with the journal retained.
3. Terminal and prepare authority still records pre-authority database file facts. Both normal and
   crash sealing calculate database bytes/SHA before `_store_terminal_seal` inserts and checkpoints the
   seal row, so the stored SHA/byte count is not the exact final database file. The public authenticator
   compares that SHA only between MANIFEST and the seal row; it never compares live database bytes or
   SHA. Preparation likewise measures its database baseline before inserting the baseline row. In all
   three store helpers, checkpoint failure occurs after COMMIT inside a handler that then executes
   ROLLBACK, which can mask the durability error while leaving committed row/file state. This is not a
   resumable, immediately immutable-readable final seal.
4. Recursive tree symlink rejection remains incomplete. Sealing silently skips symlinks under
   controller/raw/spool roots. Authentication resolves each listed candidate before `is_symlink` and
   also skips symlinks during orphan enumeration, so a nested or root symlink outside the stage
   directory is not rejected as the authorization requires.
5. Required decisive tests remain placeholders or absent. The “foreign-plan START” test inserts only a
   foreign plan row and no foreign policy/permit/stage/START graph. There is no real startup failure
   replay, no committed-success cleanup-fault replay, no full-row divergence matrix, and no assertion
   that live final database bytes/digest agree after seal/baseline insertion. These omissions cannot be
   replaced by nonempty fields or clean static checks.

Syntax compilation, targeted ruff, repository control, and `git diff --check` pass. The seven-file
source boundary is exact. Sol ran no pytest, migration, RPC, production-data mutation, or Git action.
...
`opencode.json`, both untracked research files, logs, databases, and every unrelated path. After that
publication, Sr Dev - Grok Build remains at the Grok 4.6 Max escalation tier and may correct only the
same seven authorized files:

- Implement one explicit production-journal recovery state machine for successful and failed
  acquisitions. It must distinguish not-committed, committed-before-cleanup, and partial-cleanup
  states; authenticate stable journal-bound acquisition, catalog, attempt, and lineage records in
  full; retain unresolved journals; and fail production startup on malformed or divergent authority.
  All authoritative timestamps/bytes/outcomes needed for exact replay must be durable rather than
  regenerated during replay.
- Make terminal and ARMED publication resumable across every row/COMMIT/checkpoint/file boundary.
  A clean `mode=ro&immutable=1` connection must authenticate immediately with no WAL/SHM dependency.
  Bind and remeasure the exact final live database bytes and the non-self-referential governed content
  digest after authority insertion; do not label a pre-insert physical SHA as the final database SHA.
  Preserve the source-pinned authority DDL and coordinated trigger/row/file forgery rejection.
- Reject root and nested symlinks before resolution during recursive-tree measurement and again during
  authentication; do not skip them from inventory or orphan checks. Preserve the corrected public
  attach/chain/run/drain lifecycle, natural useful indexes, malformed-readiness normalization, atomic
  catalog boundary, and every already-decisive tamper/rollback test.
- Replace the remaining nominal tests with real public-path graphs and injected boundaries: successful
  and failed startup replay; crash after commit before cleanup and between cleanup operations; exact
  divergence of every acquisition/attempt/lineage authority field with journal retention; a genuinely
  foreign policy/permit/stage/START chain; final seal/baseline immediate immutable authentication and
  live byte/digest comparison; checkpoint/file tear recovery; and root/nested symlink refusal. No
  direct-persist substitute, empty second startup, conditional assertion, swallowed production error,
  or earlier-guard success is acceptable.

Sr does not run tests or migrations, edit repository records or unrelated files, use RPC/network,
touch the offline production database or evidence, or perform Git actions. Sr stops for a fresh Sol
source review with new hashes. Jr reintegration/testing and migration 0021 remain unauthorized until
that review. All live readiness, production prepare/RPC/stages, coverage, publication, downstream,
...
`Sol source review - sixteenth controller correction rejected (2026-08-17)`. Jr Dev - Hermes is the
next actor only to publish this aligned two-record review. The seventeenth correction becomes available
to Sr Dev - Grok Build at Grok 4.6 Max only after that publication and must stop for fresh Sol source
review. Jr reintegration/testing, migration 0021, live readiness, production preparation/RPC/stages,
coverage, publication, downstream, PAPER, LIVE, harmonic-model, and next-ticket work remain
unauthorized. Next ticket remains `NONE`.

## Sol source review - seventeenth controller correction rejected (2026-08-17)

Jr published the sixteenth-correction rejection and seventeenth-correction authorization at pushed
commit `395c6bf`; `HEAD == origin/main`. Sr delivered the seventeenth correction inside the same
seven authorized files. Reviewed SHA-256 values are:
...
The correction materially improves the prior drop. Production journals now enter a dedicated startup
recovery path, malformed recognized production authority fails closed, committed catalog/attempt/lineage
presence is reconciled, nested recursive-tree symlinks are rejected, WAL checkpoint failures are surfaced
through `flush_to_main`, and a real foreign plan/policy/permit/stage/START row graph is present. These
improvements are retained. Acceptance is nevertheless blocked by the following seventeenth-scope defects:

1. Exact replay authority is still created after the transaction it is supposed to recover.
   `_seal_committed_journal_snapshot` runs only after raw/attempt/lineage COMMIT. A crash before it leaves the
   original journal; missing/unreadable journal or attempt state makes the helper silently return; its temp
   write/replace has no file or directory fsync. `_authenticate_committed_production_replay` treats
   `committed_snapshot` as optional, so this real commit-before-snapshot window accepts only a partial identity.
   The snapshot must be durably frozen before the commit boundary, not retrofitted afterward.
2. Even the committed authenticator is not byte-for-byte. It does not compare catalog status/request/response
   metadata; attempt domain, block, latency, and unsnapshotted terminal fields; or lineage plan/stage/raw/status/
   created values against journal authority. Its lineage ID recomputation uses the existing row's raw/status
   values as its own expected input. Thus a self-consistent divergent row can authenticate. The new divergence
   test mutates only attempt status and replants the pre-snapshot journal, so the source has no branch that can
   detect the mutation it promises will raise.
3. Cleanup reverses the prior unsafe order but does not recover its own real partial state. Journal is now
   unlinked before spool. A crash between those operations leaves an orphan `response-*.spool` with no journal;
   startup counts it permanently against capacity but neither authenticates nor removes it. The new partial-
   cleanup test constructs the opposite state—journal present and spool absent—which cannot result from the
   implemented ordering. Exact between-operation resumability remains unproven and incomplete.
4. The provisional final-seal algorithm repeats the stale-file-identity defect one mutation later. It inserts
   and flushes a provisional seal, measures database SHA/bytes, writes MANIFEST, then updates
   `database_sha256`, `database_bytes`, `full_manifest_hash`, and `file_identity_final` in that database and
   flushes again. The recorded SHA therefore precedes the final database update. Authentication compares the
   stale SHA only between seal and MANIFEST and never hashes the live database. Its new test checks live bytes
   and logical digest but deliberately does not compare live SHA. Prepare baseline uses the same provisional-
   row/finalizing-update pattern and does not remeasure the full tree after final mutation.
5. `file_identity_final` is a refusal flag, not resumability. There is no recovery entry point for a crash after
   terminal/baseline provisional insert, MANIFEST write, finalizing update, or before TERMINAL publication; a
   retry encounters already-mutated terminal/stage authority rather than deterministically completing or
   rolling back the same seal. No checkpoint/file-tear injection test exists despite its explicit authorization.
6. Root symlinks remain accepted by stage sealing. `_tree_inventory` resolves `root_path` before any symlink
   check and examines only descendants; a policy-bound raw/spool/controller root that is itself a symlink is
   inventoried under its target. The manifest then stores the resolved target, so authentication cannot recover
   the hidden root alias. The new test covers only a nested raw-file symlink; the existing root test exercises
   the CLI measurement helper, not stage sealing/authentication.
7. Several tests cannot establish their stated contracts. `_seed_production_stage` broadly catches and ignores
   policy, permit, and stage-construction failures, then substitutes an arbitrary first stage. The coordinated
   seal and baseline delete/reinsert tests omit the new mandatory `file_identity_final` column from their INSERTs
   and fail at NOT NULL enforcement before the intended authentication assertion. The foreign START is appended
   directly to a PREPARED stage rather than produced by a valid public authorize/start lifecycle. No full-field
   acquisition/attempt/lineage matrix, actual cleanup-order boundary, live SHA equality, or tear recovery is
   present.
...
- Freeze the complete deterministic success/failure replay record durably before the atomic database commit can
  become visible. Commit, catalog, attempt, and lineage authentication must compare every authoritative field to
  that record; absence, truncation, or divergence retains evidence and fails startup. No optional post-commit
  snapshot, silent return, row-derived expectation, or swallowed construction error is permitted.
- Implement and test the actual cleanup state machine in its real operation order, including crash before either
  unlink, between unlinks, and after both. Every orphan spool/journal must be deterministically attributable,
  safely cleaned only after exact committed authentication, or retained with startup failure; it must never leak
  capacity or be reclassified.
- Replace the circular provisional/finalizing update with a seal/baseline protocol whose claimed final file facts
  are measured after the last database mutation they describe. Public immutable authentication must recompute
  every claimed live fact, including physical SHA if it remains an authority field. Add a real idempotent recovery
  entry point for every row/COMMIT/checkpoint/MANIFEST/TERMINAL boundary and prove retry reaches one identical
  authenticated terminal or remains visibly failed with no silent partial success.
- Reject policy-bound root symlinks before resolution at seal and authentication, as well as nested symlinks.
  Replace broad-catching helpers and invalid direct row substitutes with valid public authority graphs. Repair
  every INSERT for the actual migration schema and add the complete field-divergence, true cleanup-order, root-
  symlink, live-final-SHA/bytes/digest, and injected tear-recovery matrix. Preserve all accepted atomic catalog,
  lifecycle/drain, readiness, natural-index, source-pin, nested-symlink, and coordinated-forgery behavior.
...
Jr published the eighteenth-correction rejection and nineteenth-correction authorization at pushed commit
`880c509`; `HEAD == origin/main`. Sr delivered the nineteenth correction inside the same seven authorized files.
Reviewed SHA-256 values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `d19afce7f7cdad15e07bc966d2813f3b96af550134c2919933d592c5deb9327c`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
  `f0d04d0ca2572a88757fdb9ef02c2f50915561e054e12563443d730faa70e15b`;
- `scripts/research/run_uniswap_v2_pair_events_v2_production.py`:
  `4a9aeb260bf085fb55d15f18432ab3c196ecce9aba6407a636b4cbc898fae06c`;
- `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `be950e9700a95c828c0c5a2aee8e391581316a0aa7eff427c6c31f5c921fd13f`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `129da81c4e9191f36850e5a0e6bab1db666a0461a5595ec3e67b74f490b75726`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
  `63931f0c42210c0b2f462284276a425a407c6e451eed204b143e7fa689106b01`; and
- `tests/acquisition/test_uniswap_v2_pair_events_v2_migration_0021.py`:
  `66d9a86ce409601380b29cf9c936fe3013ed4f07a2a80261bdc325a15bf5a308`.

The correction materially closes the prior findings. Snapshot-before-COMMIT success and failure now replay the
frozen catalog, attempt, and lineage cells directly and the live-row divergence matrix names all 46 declared
columns. Impossible physical SHA/byte/manifest fields and the misleading final flag are removed from immutable
seal/baseline rows; live physical identity remains in remeasured MANIFEST/TERMINAL or PREPARE_OUTCOME files.
Terminal publication has a durable PUBLICATION intent and recovery path, ARMED baseline recovery is public, root
and nested symlink refusal remains present, and public tests inject multiple publication/prepare boundaries.
These improvements are retained. Acceptance is still blocked:

1. Missing-transaction recovery promotes an unauthenticated snapshot. `_commit_frozen_replay_record` checks only
   snapshot version, map shape, and column presence, then inserts all three rows. It does not first bind catalog
   acquisition identity, attempt plan/stage/provider/role/logical call/number/start, lineage identities, or
   cross-row acquisition/raw/status relationships to the journal's `production_authority`. It also does not
   recompute attempt/lineage IDs before mutation. A divergent but schema-valid frozen snapshot can therefore
   become durable authority and immediately proceed to cleanup instead of being retained and refused.
2. Successful frozen replay can register unauthenticated raw evidence. When the referenced `raw_object` row is
   absent, `_insert_frozen_replay_rows` derives a SHA from the claimed `raw_object_id`, stats the expected path,
   and inserts the row without hashing the file, comparing its bytes to the attempt/catalog authority, or running
   publication-receipt verification. When a row already exists, it checks only presence, not any raw-object
   field. Corrupt or divergent retained bytes/metadata can thus be cataloged as a successful production attempt;
   the journal and spool are then deleted. This violates provenance and no-silent-partial-success requirements.
3. The new tests establish exact replay only for an unmodified snapshot and exact divergence only after committed
   database rows exist. They do not corrupt each frozen snapshot map in the all-rows-absent path, break internal
   cross-row/deterministic identities, corrupt the retained raw file, or preseed a divergent `raw_object` row and
   prove journal retention with zero inserted authority. The nominal 46-field matrix cannot cover this distinct
   recovery branch.
4. Several claimed COMMIT/checkpoint injections occur too late. `after_terminal_row` is invoked only after
   `store_terminal` has committed and completed `flush_to_main`; `after_seal_row` follows seal COMMIT and flush;
   `after_baseline_row` likewise follows baseline COMMIT and flush. `after_flush` is after an ordinary PRAGMA in
   the caller or after the already-flushed seal. No test injects the real row-COMMIT-before-checkpoint/flush
   window, and checkpoint errors are still swallowed at several drain boundaries. The explicit durability tear
   requirement is therefore not implemented or proven.
5. Recovery mutates before authenticating its intent. `recover_stage_terminal_publication` accepts
   `PUBLICATION.json` without first binding its plan/stage/final state/checkpoint/event root/report/outcome and
   next-authority fields to durable rows, transitions the stage, and directly deletes any plan lease while
   swallowing SQLite errors. A divergent intent can poison terminal state or delete a newer/foreign lease before
   later authentication fails; the accepted capability-bound/foreign-lease behavior is not preserved in this
   recovery path.
...
- Before inserting an all-rows-absent frozen replay, authenticate every snapshot field and cross-row relationship
  against the durable journal authority and deterministic IDs. Absence, truncation, internal inconsistency, or
  divergence must leave journal/spool evidence intact and insert no catalog/attempt/lineage authority.
- Authenticate the exact retained raw object before successful replay: regular no-follow path, expected content
  address, live SHA-256, byte size, storage identity, and every existing/new `raw_object` field. A corrupt file or
  divergent preexisting row must fail startup without cleanup or partial database mutation.
- Add decisive all-rows-absent tests for snapshot-map/identity/cross-row divergence, raw-byte corruption, and a
  divergent existing raw-object row, alongside the retained complete live-row matrix. Assert zero partial rows
  and retained evidence for every refusal.
- Place failure injection at the actual COMMIT-before-`flush_to_main`/checkpoint boundary for terminal, seal, and
  baseline writes, and prove recovery after a genuinely failed checkpoint as well as visible fail-closed states.
  Authenticate PUBLICATION intent fully before any stage/lease mutation; preserve capability/lease ownership and
  never swallow a lease mutation error. Preserve the corrected honest schema, external live-file measurements,
  idempotent file recovery, cleanup state machine, symlink refusal, public lifecycle, and source-pinned authority.
...
## Sol source review - twenty-first controller correction rejected (2026-08-17)

Jr published the twentieth-correction rejection and twenty-first-correction authorization at pushed commit
`e987b9e`; `HEAD == origin/main`. Sr delivered the twenty-first correction inside the same seven authorized
files. Reviewed SHA-256 values are:

- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_engine.py`:
  `7a616867919604153ad092cb411506bc3e20903cb668b996ecbce6cb43285b44`;
- `src/cryptofactors/acquisition/uniswap_v2_pair_events_v2_production.py`:
  `42194c1da30614873d9cb87ef75aa0d30992b42006a0cd2a84ce87c4725eac5d`;
- `scripts/research/run_uniswap_v2_pair_events_v2_production.py`:
  `4a9aeb260bf085fb55d15f18432ab3c196ecce9aba6407a636b4cbc898fae06c`;
- `sql/migrations/0021_uniswap_v2_pair_event_v2_production_control.sql`:
  `be950e9700a95c828c0c5a2aee8e391581316a0aa7eff427c6c31f5c921fd13f`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_engine.py`:
  `0d0192e3cff7d82c66996a4af6c87dc446872be0ffc558813d56466e14280140`;
- `tests/acquisition/test_uniswap_v2_pair_events_v2_production.py`:
  `ae0f12b45c0749a8bb0bb1b1a338172d675d31f603471a0104a9e0ff100b0224`; and
- `tests/acquisition/test_uniswap_v2_pair_events_v2_migration_0021.py`:
  `66d9a86ce409601380b29cf9c936fe3013ed4f07a2a80261bdc325a15bf5a308`.

The correction closes the detached-success-attempt branch, freezes a raw-object map, restores `ACQUIRED`
content status on absent-row replay, compares every recovered row in nominal success/failure cases, and adds a
real call-site checkpoint failure by arming `flush_to_main` after terminal COMMIT. PUBLICATION now carries lease
fields, rejects the tested newer same-stage lease, and deletes with the full lease tuple. These improvements are
retained. Acceptance remains blocked:

1. The frozen record still does not say whether the raw object was created in the interrupted transaction or was
   reused from an earlier acquisition. For an existing row, `_authenticate_frozen_raw_object` compares only raw
   identity, SHA-256, bytes, and URI and ignores the frozen source, name, request/response metadata, checksum,
   acquisition/event times, and status. For an absent row it inserts those unauthenticated fields. The new reuse
   test creates a current-acquisition snapshot first and only then inserts a different "first-writer" row, which
   cannot occur on the uninterrupted path: a real reused snapshot would have frozen that earlier row. It therefore
   proves acceptance of the ambiguity rather than exact first-write/reuse provenance.
2. Journal-derived authentication remains partial. Catalog response metadata is not checked for status code,
   retained bytes, truncation, or error kind/detail; attempt domain/block are not checked against production
   authority; and attempt outcome is not checked for logical call, truncation, stage, attempt number, or the failed
   acquisition link. Other deterministic catalog fields, including the original name and success/failure
   representation, are likewise not reconstructed and compared. Schema-valid journal divergence can still be
   inserted and cleaned up. The test matrix changes only seven coarse fields and does not cover these cells.
3. PUBLICATION does not durably bind all mutation authority. `next_authority_requirement` and the lease tuple are
   outside `core_manifest` and `report_hash`, and the event lookup reads but never requires an `END` event. A
   changed sidecar can substitute the next authority or copy a current/new lease tuple and pass authentication;
   there is no immutable intent-time identity proving which lease was present when the file was written.
4. Publication authentication and mutation are not atomic. Recovery authenticates the lease, commits the stage
   transition separately, then conditionally deletes the lease without checking affected-row count. A lease can
   change between those operations; recovery then reports success with the newer lease retained, or leaves the
   stage transitioned if deletion fails. The crash-residue path also still deletes every lease for a plan and
   swallows SQLite errors rather than binding an exact generation/controller/capability.
5. Tests do not exercise a real two-acquisition content reuse chronology, non-content raw-row/snapshot tamper,
   next-authority or lease-sidecar substitution, non-END event substitution, or a lease change between
   authentication and deletion. The shared zero-partial-authority assertion also searches attempts by
   acquisition ID, so it cannot detect an erroneously inserted NULL-linked attempt.

Syntax compilation, targeted ruff, repository control, and `git diff --check` pass. The seven-file source
boundary is exact. Sol ran no pytest, migration, RPC, production-data mutation, or Git action.

### Authorized twenty-second correction - same seven files only
...
- Version the frozen replay receipt and bind an explicit, authenticated raw disposition: created by this
  transaction versus reused preexisting content. Freeze the exact row actually observed inside the transaction.
  On reuse, require every live raw-object column to equal that frozen earlier authority; on creation, derive and
  authenticate every new row field from the journal/catalog contract before insertion. Retain live no-follow
  bytes/SHA/size/URI verification and never overwrite first-writer provenance.
- Build and compare the complete deterministic catalog, attempt, outcome, lineage, and cross-row expectations
  from the journal, including all response metadata and optional domain/block authority. A single divergent cell
  must refuse before mutation, retain evidence, and leave no catalog, attempt, lineage, or newly created raw row.
- Give PUBLICATION an immutable/durable identity that includes the exact intent-time lease tuple,
  `next_authority_requirement`, terminal outcome/final state, checkpoint, and authenticated END-event root.
  Recovery must authenticate that identity independently of mutable sidecar contents.
- Perform publication reauthentication, legal stage transition, and exact lease release under one database lock
  and transaction; require the conditional lease delete to affect exactly the expected row when it exists and
  roll back every mutation on drift/error. Remove the wildcard/swallowed crash-residue lease deletion or replace
  it with the same exact authority rule.
- Add public injected tests for a real first acquisition followed by a second acquisition reusing identical
  bytes and crashing before COMMIT; every raw-object column must match uninterrupted execution. Add exhaustive
  journal/raw-field refusal, sidecar/END/next-authority/lease tamper, authentication-to-delete lease-race, exact
  row-count, retained-evidence, and truly global zero-partial-row assertions.

Preserve the corrected success/failure links, honest seal/baseline schema, genuine checkpoint failure, live-file
facts, idempotent recovery/cleanup, symlink refusal, public lifecycle, and source-pin behavior. Sr does not run
tests or migrations, edit repository records or unrelated files, use RPC/network, touch the offline production
database or evidence, or perform Git actions. Sr stops for fresh Sol source review with new hashes. Jr
reintegration/testing and migration 0021 remain unauthorized. All live readiness, production prepare/RPC/stages,
coverage, publication, downstream, PAPER, LIVE, harmonic-model, and next-ticket work remain unauthorized. Next
ticket remains `NONE`.

## Latest control-plane state (2026-08-17)

The authoritative current decision is the immediately preceding section,
`Sol source review - twenty-first controller correction rejected (2026-08-17)`. Jr Dev - Hermes is the next
actor only to publish this aligned two-record review. The twenty-second correction becomes available to Sr Dev -
